...
Serves the React frontend (../webapp, built with `npm run build` into
../webapp/dist) and a small JSON API:
  GET  /api/health      -> {ai: bool}
  GET  /api/modules     -> learning modules
  GET  /api/quiz        -> 5 questions   (Gemini AI, or offline bank on any failure)
  GET  /api/simulator   -> 5 emails      (Gemini AI, or offline bank on any failure)
  POST /api/chat        -> chatbot reply (Gemini AI, or offline answer on any failure)
...
# Serves the built React app (phishguard_web/webapp) after `npm run build`.
FRONTEND = os.path.abspath(os.path.join(HERE, "..", "webapp", "dist"))
...
# Imported after .env so GEMINI_MODEL from .env is honoured.
import fallback_data as fb                                   # noqa: E402
from gemini_client import GeminiError, ask_gemini, extract_json_array, has_key  # noqa: E402
...
def quiz():
    ...
    if has_key():
        try:
            raw = ask_gemini("You are a phishing awareness quiz generator. Only respond with valid JSON.",
...
def simulator():
    ...
    if has_key():
        try:
            raw = ask_gemini("You are a phishing awareness trainer. Only respond with valid JSON.",
...
def chat():
    ...
    if has_key():
        try:
            reply = ask_gemini(CHAT_SYSTEM, messages, temperature=0.5).strip()
            if reply:
...
if __name__ == "__main__":
    ...
    print(f"  AI:    {'Gemini key found' if has_key() else 'no GEMINI_API_KEY - using offline question bank'}")
    print("  Stop:  Ctrl+C\n")
