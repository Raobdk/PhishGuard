...
const MENU = [
  ...
  ["lab", "🎣", "Victim / Attacker Lab", "Two boxes, side by side, live. Launch the attack, then get phished, and watch."],
];
  ...
  async function ask(text) {
    ...
    try {
      ...
      const note = data.source === "offline" && !warnedOffline
        ? (data.notice || "Offline mode: short built-in answers. Add a GEMINI_API_KEY for the full AI.") : null;
...
// ---------------------------------------------- victim / attacker lab (dual box)
// Both boxes render at once and stay on screen together. Nothing here is a real
// exploit: the "attacker" commands are cosmetic text, the IP is the IANA block
// reserved for documentation examples (RFC 5737), and no request ever leaves
// the browser. It exists to show, live, how the two sides of one phishing
// attempt connect - not to hand out a working phishing kit.
screens.lab = async (root, alive) => {
  ...
  const FAKE_IP = "203.0.113.42"; // TEST-NET-3 (RFC 5737): reserved for docs/examples, never a live host
  let run = 0;                    // bumps on (re)start so old async loops stop themselves
  const liveRun = (r) => alive() && r === run;

  function start() {
    run++;
    const r = run;
    if (scene) scene.setMood("lab");
    root.replaceChildren();

    const mail = lab.phishing;

    // ---------------------------------------------------------- attack box
    const atkTag = h("span", { class: "pane-tag", text: "idle" });
    const atkTerm = h("div", { class: "terminal mini", role: "log", "aria-label": "Simulated attacker terminal" });
    const launchBtn = h("button", {
      class: "btn danger",
      text: "▶ Launch phishing campaign",
      onclick: () => { launchBtn.remove(); runAttackerScript(); },
    });
    const atkPane = h("div", { class: "lab-pane" },
      h("div", { class: "lab-pane-head" }, h("h3", { text: "🖥 Attack Box" }), atkTag),
      atkTerm,
      h("div", { class: "row" }, launchBtn));

    // ---------------------------------------------------------- victim box
    const vicTag = h("span", { class: "pane-tag", text: "offline" });
    const vicScreen = h("div", { class: "phone-screen" },
      h("div", { class: "waiting" },
        h("div", { text: "💤" }),
        h("p", { text: "Waiting for the attack box to start the campaign…" })));
    const vicPane = h("div", { class: "lab-pane" },
      h("div", { class: "lab-pane-head" }, h("h3", { text: "📱 Victim Box" }), vicTag),
      h("div", { class: "phone" }, vicScreen));

    root.append(
      h("h1", { class: "screen-title", text: "🎣 Victim / Attacker Simulation Lab" }),
      h("p", { class: "screen-sub", text: "Two boxes, running side by side. Launch the campaign from the attack box, then act as the victim on the right - watch what lands back on the left the instant you submit anything." }),
      h("div", { class: "lab-split" }, atkPane, vicPane),
      h("div", { class: "spacer" }),
      backButton("Menu"),
    );

    // ---- attacker terminal helpers ----
    const scrollTerm = () => { atkTerm.scrollTop = atkTerm.scrollHeight; };
    const line = (cls = "") => { const el = h("div", { class: `ln ${cls}` }); atkTerm.append(el); return el; };
    const blank = () => atkTerm.append(h("div", { class: "ln" }, " "));
    ...
    async function runAttackerScript() {
      atkTag.textContent = "running"; atkTag.classList.add("live");
      const script = [
        [`attacker@attackbox:~$ campaign start --brand="${mail.brand}" --clone=login`, "", 12],
        ["[*] cloning login page ... done", "dim", 8],
        [`[*] spoofing sender address: ${mail.from_email}`, "dim", 8],
        [`[*] binding simulated listener on ${FAKE_IP}:8080 (test-only address, not routable)`, "", 8],
        ["[*] queuing phishing email into victim inbox ...", "", 10],
        ["[+] email delivered. listening for the victim to open it ...", "good", 10],
      ];
      for (const [t, c, sp] of script) { if (!liveRun(r)) return; await typeLine(t, c, sp); }
      if (!liveRun(r)) return;
      vicTag.textContent = "inbox: 1 new"; vicTag.classList.add("live");
      showInbox();
    }

    // ---- victim-box screens (all inside vicScreen) ----
    function setVictim(...nodes) { vicScreen.replaceChildren(...nodes); }

    function showInbox() {
      const row = (m, onOpen) => h("button", { class: "inbox-row", onclick: onOpen },
        h("div", { class: "from", text: m.from_name }),
        h("div", { class: "subj", text: m.subject }),
        h("div", { class: "prev", text: m.preview }));
      setVictim(
        h("div", { class: "phone-inbox" },
          row(mail, showPhishingEmail),
          row(lab.safe, showSafeEmail)));
    }
    ...
    function showSafeEmail() {
      ...
      setVictim(
        ...
        h("button", { class: "btn", onclick: showInbox, text: "← Back to inbox" }));
    }

    function showPhishingEmail() {
      const status = h("div", { class: "statusbar", text: " " });
      const cta = h("button", { class: "email-cta", onclick: showLogin, text: `${mail.link_text} →` });
      cta.addEventListener("pointerenter", () => { status.textContent = mail.fake_url; });
      cta.addEventListener("pointerleave", () => { status.textContent = " "; });
      setVictim(
        h("div", { class: "email" },
          ...
          status),
        h("button", { class: "btn ghost", onclick: showInbox, text: "← Back to inbox" }));
    }
    ...
    function showLogin() {
      ...
      const form = h("form", {
        onsubmit: (e) => {
          ...
          onCredentialsSubmitted(captured);
        },
      },
    ...
    // The moment the "victim" submits, the attacker box reacts live - no extra
    // click needed to "see what the attacker sees".
    function onCredentialsSubmitted(captured) {
      setVictim(h("div", { class: "waiting" }, h("div", { text: "🔄" }), h("p", { text: "Redirecting… (the attack box is watching this happen right now)" })));
      if (scene) scene.setMood("attack");
      captureOnAttacker(captured);
    }

    async function captureOnAttacker(captured) {
      vicTag.textContent = "captured"; vicTag.classList.remove("live");
      blank();
      const domain = mail.from_email.split("@").pop();
      const script = [
        [`[+] connection received from victim @ ${FAKE_IP}`, "", 8],
        [`POST /verify HTTP/1.1  Host: ${domain}`, "dim", 5],
        ["Content-Type: application/x-www-form-urlencoded", "dim", 5],
        ["[+] form intercepted, parsing credentials ...", "", 10],
      ];
      for (const [t, c, sp] of script) { if (!liveRun(r)) return; await typeLine(t, c, sp); }
      if (!liveRun(r)) return;
      ...
      blank();
      line("dim").textContent = "[!] this is where a real kit would exfiltrate the loot - this one just stops here.";
      scrollTerm();
      captured.user = captured.password = ""; // drop it from memory now that the demo is over
      if (scene) scene.flash("bad");
      atkTag.textContent = "captured";

      setVictim(h("div", { class: "feedback bad" }, h("strong", { class: "bad", text: "🎣 You just got phished. " }),
        "Look at what showed up on the attack box the instant you hit submit."));
      showDebrief(mail);
    }

    function showDebrief(m) {
      const fakeDomain = m.from_email.split("@").pop();
      root.append(
        h("div", { class: "panel" },
          h("h3", { text: "Signs this email was phishing:" }),
          h("ul", { class: "flag-list" }, m.red_flags.map((f) => h("li", { text: f }))),
          h("div", { class: "compare" },
            h("div", { class: "real", text: `✅ Real bank: ${m.real_domain}` }),
            h("div", { class: "fake", text: `🚩 This email: ${fakeDomain}` }))),
        h("div", { class: "spacer" }),
        h("div", { class: "row" },
          h("button", { class: "btn primary", onclick: start, text: "↺ Replay the lab" }),
          h("button", { class: "btn", onclick: () => navigate("learn"), text: "📚 Learning modules" }),
          backButton("Menu")));
    }
  }
...
async function loadBadge() {
  try {
    ...
    badge.textContent = ai ? "● AI: Gemini connected" : "● Offline mode (no API key)";
    badge.className = `badge ${ai ? "ok" : "warn"}`;
    badge.title = ai ? "Quiz, simulator and chat use the Gemini AI."
      : "Quiz, simulator and chat use a built-in question bank. Add GEMINI_API_KEY to enable AI.";
  } catch (_) {
